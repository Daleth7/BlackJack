...
class BJPlayer{
    ...
    //Read-only
        ...
        Deck::Card_ptr last_card(size_t hand_index = 0)const;
        size_t hand_value(size_t hand_index = 0)const;
    //Modifiers
        void add_card(const Deck::Card_ptr&, size_t hand_index = 0);
        void clear_hand(size_t hand_index = 0);
        void clear_all_hands();
        Money bet(Money);
        void add_money(Money);
            //Return whether or not split was successful
        bool split(
            size_t hand_index = 0,
            Deck::Card_ptr = nullptr,
            Deck::Card_ptr = nullptr
        );
